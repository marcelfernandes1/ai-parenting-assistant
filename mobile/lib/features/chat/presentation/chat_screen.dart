...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  /// Requests permission if needed and triggers iOS system dialog.
  /// Returns true if permission is granted, false otherwise.
  Future<bool> _ensureMicPermission() async {
    try {
      // 1) Check current status
      var status = await Permission.microphone.status;
      debugPrint('🎤 Mic status before request: $status');

      // 2) If granted, we're done
      if (status.isGranted) {
        debugPrint('🎤 Mic permission already granted');
        return true;
      }

      // 3) If "denied" (first time or soft deny), REQUEST -> this triggers the iOS popup
      if (status.isDenied || status.isLimited) {
        debugPrint('🎤 Requesting microphone permission (iOS dialog should appear)...');
        ...
        // If user denies here, fall through to Settings banner below
        status = result;
      }

      // 4) If permanently denied / restricted, iOS will NOT show the dialog again
      if (status.isPermanentlyDenied || status.isRestricted) {
        debugPrint('🎤 Mic permission permanently denied or restricted');

        // Show CTA to open Settings
        if (mounted) {
          final messenger = ScaffoldMessenger.of(context);
          messenger.showSnackBar(
            const SnackBar(
              content: Text('Microphone permission is required. Please enable it in Settings.'),
  ...
  /// Shows ChatGPT-style recording modal with waveform animation
  /// Tap once to start recording, tap checkmark to send or X to cancel
  Future<void> _showRecordingModal() async {
    ...
    await showModalBottomSheet(
      ...
      builder: (context) => const _RecordingModal(),
    );
  }
...
/// ChatGPT-style recording modal with animated waveform
/// Shows waveform animation while recording with X (cancel) and checkmark (send) buttons
class _RecordingModal extends ConsumerWidget {
      ...
      child: SafeArea(
        child: Column(
          ...
          children: [
            ...
            // Duration counter
            Text(
              _formatDuration(voiceState.duration),
                    ...
                    onPressed: () async {
                      ...
                      if (context.mounted) {
                        ...
                        if (audioPath != null) {
                          ...
                          await chatNotifier.sendVoiceMessage(audioPath);
                        }
                      }
                    },
                    iconSize: 48,
